    ...
    <div>
        <v-card flat class="mb-12">
            ...
            <v-card-text>
                <v-row class="mt-0">
                    <v-col cols="8">
                        <v-text-field dense v-model="availableWater" outlined disabled
                            label="Available Water/Sewer"></v-text-field>
                    </v-col>
                </v-row>
                <v-row v-if="!availableWater" justify="center" class="pb-10">
                ...
                <div v-if="availableWater == 'CSWR Florida - City of Clermont - FL'">
                    <v-row justify="center">
                        <p class="green  black--text text-h6 font-weight-bold pa-0 ma-0">We will set up the Water Account w/
                            CSWR</p>
                    </v-row>
                    <v-row class="pt-5 pb-10 " justify="center">
                        <mark>
                            <h2>*Customer instructions for Sewer/Trash w/ City of Clermont will be in email*</h2>
                        </mark>
                    </v-row>
                </div>
                <v-row v-if="availableWater" class="mt-10">
                    <v-col cols="6">
                        <v-autocomplete label="Activated Water Provided" dense v-model="activatedWaterSelected"
                            @change="searchProvider()" hint="Select ONLY if we are activating the customer" outlined
                            persistent-hint :items="activatedWaterItems"></v-autocomplete>
                    </v-col>
                </v-row>
                <v-row v-if="activatedWaterSelected=='Indiana American Water - City of Seymour - IN' && avaiableWaterAux.includes(availableWater)" justify="center">
                    <p style="font-size: large;"> <span style="color:red"><b> Important:</b></span> <mark>We will be setting
                            up <b>only the water portion</b> with <b>Indiana American Water</b> for the customer.
                            <b>The customer will receive instructions in their confirmation email</b> to set up the <b>sewer
                                and trash</b> through <b>City of Seymour.</b></mark></p>
                </v-row>
                <div v-if="(activatedWaterSelected == 'Colorado Springs Utilities' || activatedWaterSelected == 'City of Columbia - SouthWest Water Company - SC')&& avaiableWaterAux.includes(availableWater)" class="pt-10 pb-10">
                    <v-row>
                        <p style="font-size: large;color:black"><b style="color:red">**IMPORTANT**</b> Please insure you
                            have the following information for the Secondary as the provider requires Emergency Contact
                            Info:</p>
                    </v-row>
                    <v-row>
                        <p>
                ...
                <v-row v-if="availableWater">
                    <v-col cols="6">
                        <v-select clearable outlined dense label="Who's Name will Water be in?" v-model="whosNamewillWater"
                            :items="['Primary', 'Secondary']"></v-select>
                    </v-col>
                </v-row>
                <v-row v-if="whosNamewillWater == 'Primary' && driverLicenseCRM">
                    <v-col cols="6">
                        <v-text-field label="Driver's Licence Number (Primary)" dense hint="Enter Number and State"
                            persistent-hint outlined v-model="driversLicenceNo"></v-text-field>
                    </v-col>
                </v-row>
                <v-row v-if="whosNamewillWater == 'Secondary' && driverLicenseCRM">
                    <v-col cols="6">
                        <v-text-field label="Driver's Licence Number (Secondary)" dense outlined
                            v-model="driversLicenceNoSecondary"></v-text-field>
                    </v-col>
                </v-row>
                <v-row v-if="ssnCRM">
                    <v-col cols="6">
                        <v-text-field label="SSN for Water/Sewer" dense outlined v-model="SSNwaterSewer"></v-text-field>
                    </v-col>
                </v-row>
                <v-row v-if="last4CRM">
                    <v-col cols="6">
                        <v-text-field prefix="123" label="Last 4 of SSN for Water/Sewer" dense outlined
                            v-model="last4SSN"></v-text-field>
                    </v-col>
                </v-row>
                <v-row
                    v-if="(this.$store.state.stepSeven.primaryDOBFormatedStore == '' && (this.$store.state.stepThree.primaryBirthDateFormattedStore == null || this.$store.state.stepThree.primaryBirthDateFormattedStore == '') && whosNamewillWater == 'Primary' && availableWater) && dobCRM"
                    dense class="mt-0">
                    <v-col cols="6" lg="6">
                        <v-menu ref="primaryDOBMenu" v-model="primaryDOBMenu" :close-on-content-click="false"
                            transition="scale-transition" offset-y max-width="290px" min-width="auto">
                            <template v-slot:activator="{ on, attrs }">
                                <v-text-field v-model="primaryDOBFormatedStore" label="Primary DOB" dense outlined
                                    hint="MM/DD/YYYY" persistent-hint append-icon="mdi-calendar" v-bind="attrs"
                                    @blur="primaryDOBDate = parseDate(primaryDOBFormatedStore)" v-on="on"></v-text-field>
                            </template>
                            <v-date-picker v-model="primaryDOBDate" no-title
                                @input="primaryDOBMenu = false"></v-date-picker>
                        </v-menu>
                    </v-col>
                </v-row>
                <v-row
                    v-if="(this.$store.state.stepThree.secondaryBirthDateFormattedStore == '' && whosNamewillWater == 'Secondary' && availableWater) && dobCRM"
                    dense class="mt-0">
                    <v-col cols="6" lg="6">
                        <v-menu ref="SecondaryDOBMenu" v-model="SecondaryDOBMenu" :close-on-content-click="false"
                            transition="scale-transition" offset-y max-width="290px" min-width="auto">
                            <template v-slot:activator="{ on, attrs }">
                                <v-text-field v-model="secondaryDOBFormatedStore" label="Secondary DOB" dense outlined
                                    hint="MM/DD/YYYY" persistent-hint append-icon="mdi-calendar" v-bind="attrs"
                                    @blur="secondaryDOBDate = parseDate(secondaryDOBFormatedStore)"
                                    v-on="on"></v-text-field>
                            </template>
                            <v-date-picker v-model="secondaryDOBDate" no-title
                                @input="SecondaryDOBMenu = false"></v-date-picker>
                        </v-menu>
                    </v-col>
                </v-row>
                <v-row v-if="requiresPINCRM" justify="center">
                    <p style="font-size:large;color:black"> <span style="color:red">Requires the customer to create a
                            <u><b>4 Digit Pin</b></u> to secure their account</span></p>
                </v-row>
                <v-row v-if="requiresPINCRM">
                    <v-col cols="6">
                        <v-text-field outlined dense label="Security Pin for Water" v-model="securityPIN"></v-text-field>
                    </v-col>
                </v-row>
                <v-row v-if="availableWater">
                    <v-col cols="6" lg="6">
                        <v-menu ref="wateractDateMenu" v-model="wateractDateMenu" :close-on-content-click="false"
                            transition="scale-transition" offset-y max-width="290px" min-width="auto">
                            <template v-slot:activator="{ on, attrs }">
                                <v-text-field v-model="waterActivationDateStore" label="Water Activation Date" dense
                                    outlined hint="MM/DD/YYYY" persistent-hint append-icon="mdi-calendar" v-bind="attrs"
                                    @blur="waterActivationDate = parseDate(waterActivationDateStore)"
                                    v-on="on"></v-text-field>
                            </template>
                            <v-date-picker v-model="waterActivationDate" no-title
                                @input="wateractDateMenu = false"></v-date-picker>
                        </v-menu>
                    </v-col>
                </v-row>
                <v-row v-if="availableWater">
                    <v-col cols="6">
                        <v-checkbox label="Haver Support Key the Water Order?" v-model="haveSupportboolean"></v-checkbox>
                    </v-col>
                </v-row>
                <v-row v-if="haveSupportboolean && availableWater">
                    <v-col cols=6>
                        <v-textarea dense outlined label="Ticket Notes" v-model="ticketNotes"></v-textarea>
                    </v-col>
                </v-row>
                <v-row v-if="availableWater">
                    <v-col cols="6">
                        <v-text-field dense outlined label="Water Tracking Number" v-model="waterTrackingNo"></v-text-field>
                    </v-col>
                </v-row>
                <v-row v-if="availableWater">
                    <v-col cols="6">
                        <v-text-field label="Water Access Key" outlined dense v-model="waterAccessKey"></v-text-field>
                    </v-col>
                </v-row>
                <v-row class="pb-5">
                ...
                <v-row>
                    <v-col cols="6">
                        <v-text-field dense outlined disabled label="Available Trash Provider(s)"
                            v-model="availableTrash"></v-text-field>
                    </v-col>
                </v-row>
                ...
                <v-row>
                    <v-col cols="6">
                        <v-autocomplete clearable dense outlined v-model="activatedTrashSelected"
                             :items="activatedTrashItems"
                            label="Activated Trash ´Provider"></v-autocomplete>
                    </v-col>
                </v-row>

                <v-row v-if="availableTrash">
                    <v-col cols="6">
                        <v-text-field outlined dense label="DL# for Trash" v-model="DLforTrash"
                            hint="Enter Number and State" persistent-hint></v-text-field>
                    </v-col>
                </v-row>

                <v-row>
                    <v-col cols="6" lg="6">
                        <v-menu ref="trashactDateMenu" v-model="trashactDateMenu" :close-on-content-click="false"
                            transition="scale-transition" offset-y max-width="290px" min-width="auto">
                            <template v-slot:activator="{ on, attrs }">
                                <v-text-field v-model="trashActivationDateStore" label="Trash Activation Date" dense
                                    outlined hint="MM/DD/YYYY" persistent-hint append-icon="mdi-calendar" v-bind="attrs"
                                    @blur="trashActivationDate = parseDate(trashActivationDateStore)"
                                    v-on="on"></v-text-field>
                            </template>
                            <v-date-picker v-model="trashActivationDate" no-title
                                @input="trashactDateMenu = false"></v-date-picker>
                        </v-menu>
                    </v-col>
                </v-row>
                <v-row class="mt-10">
            ...
            <v-row class="mt-2">
                <v-col>
                    <v-textarea outlined dense v-model="callNotes" height="100px" label="Call Notes 1"
                        hint="Who? What? When? Why? How? Next Steps?" persistent-hint></v-textarea>
                </v-col>
            </v-row>
            <v-row class="mt-2">
                <v-col cols="6">
                    <v-select clearable v-model="salesRep" label="Sales Rep *" outlined :items="salesRepItems" dense
                        hint="Select your Name for Sales Reporting" persistent-hint></v-select>
                </v-col>
            </v-row>
            <v-row class="mt-0">
                <v-col cols="6">
                    <v-checkbox dense v-model="error" label="Errors in Research?" color="red"></v-checkbox>
                </v-col>
            </v-row>
            <v-row v-if="error == true" class="mt-0">
                <v-col cols="6">
                    <v-text-field dense outlined v-model="errorField" label="What was the Research Error?"></v-text-field>
                </v-col>
            </v-row>
            <v-row class="mt-5">
                <v-col>
                    <p style="color:black;font-size:18px">Thank you for trusting Utility Helpers! We strive to provide 5
                        star service. If you found your service helpful it would mean the world to us if when you receive
                        that confirmation email, you took a moment to leave us a review about your experience today. What
                        else I can do for you today to earn your 5 Star review?</p>
                </v-col>
            </v-row>
            <!-- <v-row class="mt-4">
                <v-col>
...
export default {
    data: vm => ({
        ...
        primaryDOBFormated: '',
        //CRM Water
        ssnCRM: "", // Puedes inicializarlo con null o cualquier valor predeterminado
        last4CRM: "",
        dobCRM: "",
        requiresPINCRM: "",
        requiresQuestionCRM: "",
    ...
    methods: {
        ...
        async searchProvider() {
            ...
            const that = this;
            await ZOHO.CRM.API.searchRecord({ Entity: "Providers", Type: "criteria", Query: "(Name:equals:" + that.activatedWaterSelected + ")", delay: false })
                .then(function (data) {

                    // Optional fields requires               
